...
const CarTypePage = ({thirdslug}) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getAllCarTypeProducts(thirdslug , paramsFilter , currentPage);
        ...
        fetchData();
    }, [thirdslug , paramsFilter , currentPage]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getCarData(params.secslug);
    ...
    return(
        <>
            <Head>
                <link rel="canonical" href={"https://girpazh.com/car/" + params.slug + "/" + params.secslug + "/" + thirdslug} />
            </Head>
            <div className="container max-w-[1200px] w-11/12 mx-auto my-10">


                <div className="flex items-center flex-row-reverse gap-2 text-xs text-[#8D8989] max-md:text-[10px] max-md:gap-1">
                    ...
                    <Link href={"/car/" + brandData?.slug} className="hover:text-black duration-300">
                        برند {brandData?.name}
                    </Link>
                    <svg width="7" height="12" viewBox="0 0 7 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M6 11L1 6L6 1" stroke="#8D8989" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                    <Link href={"/car/" + brandData?.slug + "/" + carData?.slug} className="hover:text-black duration-300">
                        خودرو {carData?.name}
                    </Link>
                    <svg width="7" height="12" viewBox="0 0 7 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M6 11L1 6L6 1" stroke="#8D8989" stroke-linecap="round" stroke-linejoin="round" />
                    ...
                    <p>
                        لوازم یدکی {thirdslug}
                    </p>
                </div>

                <div className="bg-[#383838] p-3 rounded-lg mt-10 max-md:mt-5 max-md:p-1">
                    <div>
                        <h1 className="text-center text-white text-xl max-md:text-lg">
                            لوازم یدک {thirdslug}
                        </h1>
                    </div>
                </div>
                ...
                <div className="mt-10 max-md:mt-5 flex items-start gap-5">
