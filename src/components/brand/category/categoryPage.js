...
const CategoryPage = ({slug}) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getAllCategoryProducts(slug , paramsFilter , currentPage);
    ...
    return(
        <>
            <Head>
                <link rel="canonical" href={"https://girpazh.com/category/" + slug} />
            </Head>
            <div className="container max-w-[1200px] w-11/12 mx-auto my-20 mt-2 py-10 max-md:mt-1">
                ...
                <div className="grid grid-cols-6 gap-5 max-md:grid-cols-3 mt-10 max-md:grid-cols-1 max-md:5 bg-[#221F1F] p-4 rounded-lg" style={{ direction: "rtl" }}>
                    {subCategories?.map((item) => (
                        <Link href={"/category/" + slug + "/" + item?.slug} className="flex items-center gap-2 p-1 rounded-xl bg-white justify-center hover:scale-110 duration-300" style={{ border: "1px solid rgb(0,0,0,10%)" }}>
                            <div>
                                <p className="text-sm mb-1 text-center max-md:text-xs">
                                    {item?.name}
                                </p>
                            </div>
                        </Link>
