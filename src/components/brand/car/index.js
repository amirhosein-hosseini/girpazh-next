...
import { getAllCarProducts, getAllCarTypeProducts, getAllPartBrands, getAllProductCategories, getBrandData, getCarData } from "@/api/categories";
import ShopItem from "@/components/shop/shopItem";
import { getAllCategories } from "@/api/home";
import Link from "next/link";
import Head from "next/head";

const CarPage = ({secslug}) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getAllCarProducts(secslug , paramsFilter , currentPage);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getCarData(secslug);
    ...
    return(
        <>  
            <Head>
                <link rel="canonical" href={"https://girpazh.com/car/" + params.slug + "/" + secslug} />
            </Head>
            <div className="container max-w-[1200px] w-11/12 mx-auto my-10">
                ...
                <div className="flex items-center flex-row-reverse gap-2 text-xs text-[#8D8989]">
                    ...
                    <Link className="hover:text-black duration-300" href={"/car/" + brandData?.slug}>
                        برند {brandData?.name}
                    </Link>
                    <svg width="7" height="12" viewBox="0 0 7 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M6 11L1 6L6 1" stroke="#8D8989" stroke-linecap="round" stroke-linejoin="round" />
                ...
                <div className="bg-[#383838] p-3 rounded-lg mt-10">
                    <div>
                        <h1 className="text-center text-white text-xl">
                            مدل های خودرو {carData?.name}
                        </h1>
                    </div>
                </div>

                <div className="grid gap-4 grid-cols-6 mt-10 max-md:grid-cols-2" style={{ direction: "rtl" }}>
                    {carData?.car_types?.map((item) => (
                        <Link className="flex hover:scale-110 duration-300" href={"/car/" + brandData?.slug + "/" + carData?.slug + "/" + item?.slug}>
