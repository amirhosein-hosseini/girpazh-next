...
import { getAllBrandProducts, getAllCarProducts, getAllPartBrands, getAllProductCategories, getBrandData, getCarTypes } from "@/api/categories";
...
const BrandPage = ({slug}) => {
    ...
    const [paramsFilter , setParamsFilter] = useState("cat=&brand=&sort=");
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getAllBrandProducts(slug, paramsFilter , currentPage);
    ...
    // get data for carTypes
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getCarTypes(activeCarType);
    ...
    useEffect(() => {
        setParamsFilter("cat=" + filter?.cat + "&sort=" + filter?.sort + "&brand=" + filter?.brand)
    }, [filter]);
    ...
    return(
        <>
            <Head>
                <link rel="canonical" href={"https://girpazh.com/car" + slug} />
            </Head>
            <div className="container max-w-[1200px] w-11/12 mx-auto my-10">
                ...
                <div className="bg-[#383838] p-3 rounded-lg mt-10">
                    <div>
                        <h1 className="text-center text-white text-xl">
                            ماشین های برند {brandData?.name}
                        </h1>
                    </div>
                </div>

                <div className="grid gap-4 grid-cols-6 mt-10 max-md:grid-cols-2" style={{ direction: "rtl" }}>
                    {brandData?.cars?.map((item) => (
                        <Link className="flex hover:scale-110 duration-300" href={"/car/" + brandData?.slug + "/" + item?.slug}>
                            <div className="relative bg-white flex flex-col gap-5 p-3 rounded-lg pt-5">
                                ...
                                <div>
                                    <button className="bg-[#E14957] w-full text-white text-sm rounded-lg py-2">
                                        مشاهده
                                    </button>
                                </div>
                                <svg className="absolute top-0 w-6/12 left-[50%]" style={{ transform: "translate(-50%,0)" }} width="88" height="6" viewBox="0 0 88 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M0 0H88L83 6H5L0 0Z" fill="#E14957" />
                                </svg>
                            </div>
                        </Link>
                    ))}
                ...
                <button className="flex items-center gap-1 p-1 mt-5 border border-black rounded-lg ml-auto px-4 hidden max-md:hidden" onClick={() => setShowMobileFilter(!showMobileFilter)}>
                ...
                <div style={showMobileFilter === true ? { transform: "translateY(0%)", overflowY: 'scroll', boxShadow: "-2px -6px 17.1px 0px rgba(0, 0, 0, 0.17)" } : { transform: "translateY(100%)" }} className={styles.mobileCategory + " fixed bottom-0 left-0 bg-white z-50 w-full rounded-tr-2xl rounded-tl-2xl p-4 duration-300 max-h-[500px]"}>
                    ...
                    <div className="mt-2">
                        ...
                        <div className="flex flex-col gap-2">
                            {categories?.map((item) => (
                                <div className="flex items-center gap-1 flex-row-reverse" onClick={() => handleCategoryChange(item?.id)}>
                                    {filter?.cat === item?.id ?
                ...
                <div className="mt-10 flex items-start gap-5">
                    ...
                    <div className="w-9/12 flex flex-col gap-3 max-md:w-full">
                        <div className="flex items-center gap-4 bg-white rounded-lg p-3 flex-row-reverse max-md:hidden">
